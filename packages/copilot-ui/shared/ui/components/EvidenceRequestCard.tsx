...
function getCompletenessBarColor(completeness: number): string {
  ...
  // This rung read `…accent-soft` with a stray trailing zero — no such class,
  // so it rendered with no colour at all while every other rung is solid.
  // Found by the preset-token gate, which is the only thing that can see a
  // class producing no css.
  if (completeness >= 0.7) return 'bg-fm-accent';
