...
export const ErrorHandlerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [dismissalTimeouts, setDismissalTimeouts] = useState<Map<string, ReturnType<typeof setTimeout>>>(new Map());
  ...
  const timeoutIdsRef = useRef<Map<string, ReturnType<typeof setTimeout>>>(timeoutIds);
  const dismissalTimeoutsRef = useRef<Map<string, ReturnType<typeof setTimeout>>>(dismissalTimeouts);
