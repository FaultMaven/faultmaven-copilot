                  ...
                  {item.attachments && item.attachments.length > 0 && (
                    <div className="flex flex-wrap items-center gap-1.5 mt-1.5 pt-1.5 border-t border-fm-border/50">
                      {item.attachments.map((att, idx) => {
                        // Origin comes from `upload_source`, not `source_type`
                        // — the latter carries the DATA classification, so it
                        // never matched. See `attachmentOrigin` (#224).
                        const origin = attachmentOrigin(att);
                        let icon: React.ReactNode;
                        if (origin === 'page_capture') {
                          icon = (
