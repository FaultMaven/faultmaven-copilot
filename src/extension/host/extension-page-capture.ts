/**
 * Page capture, as only an extension can do it.
 *
 * This is the extension host's implementation of `HostPageCapture.capture()`.
 * It was `usePageContent`, a hook in `src/shared/ui`; nothing about it was ever
 * shared — every line reaches `tabs`, `permissions` or `scripting`, and a web
 * page cannot read another tab at all. It lives under the host now, and the
 * shared UI asks the host whether capture is possible rather than assuming it.
 *
 * The React state it used to hold (`pageContent`, `injectionStatus`) had no
 * reader: the only consumer destructured `handlePageInject` and nothing else.
 * A plain async function is what the adapter's capability union asks for, so
...
export async function capturePage(): Promise<{ content: string; url: string }> {
  try {
    ...
    // No content script implements a `getPageContent` message handler, so the
    // page extractor is always injected programmatically via
    // scripting.executeScript (ensuring host permission for the tab first).
    try {
    ...
    // The message is what the input bar shows the user, so it is re-thrown
    // rather than swallowed into a status field nobody read.
    log.error("capturePage error:", err);
    throw err;
  }
}
