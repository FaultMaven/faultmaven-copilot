...
> Note: `logoutAuth()` POSTs `/api/v1/auth/logout`, which revokes the **access** token server-side. In OAuth (cloud) mode it *also* best-effort POSTs `/api/v1/auth/oauth/revoke` (`token_type_hint: refresh_token`, RFC 7009) to revoke the **refresh** token server-side. Wrapped so any failure never blocks logout, and skipped in local mode (where `/oauth/revoke` isn't mounted). The in-browser copy is destroyed by `clearAllAuthData()` regardless.
